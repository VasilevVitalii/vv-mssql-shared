...
function go (filter) {
    ...
    let query_per_database = [
        ...
        "FROM {1}INFORMATION_SCHEMA.[COLUMNS] c WITH (NOLOCK)",
        ...
        "    FROM {1}INFORMATION_SCHEMA.TABLE_CONSTRAINTS tc WITH (NOLOCK)",
        "    JOIN {1}INFORMATION_SCHEMA.[KEY_COLUMN_USAGE] kcu  WITH (NOLOCK) ON kcu.[TABLE_SCHEMA] =  tc.[TABLE_SCHEMA] AND kcu.[TABLE_NAME] =  tc.[TABLE_NAME] AND tc.[CONSTRAINT_NAME] = kcu.[CONSTRAINT_NAME]",
        ...
        "    FROM {1}sys.identity_columns WITH (NOLOCK)",
