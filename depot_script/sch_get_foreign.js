...
function go (filter) {
    ...
    let query_per_database = [
        ...
        "FROM {1}sys.foreign_keys fk WITH (NOLOCK)",
        "JOIN {1}sys.tables fk_tab WITH (NOLOCK) ON fk_tab.object_id = fk.parent_object_id",
        "JOIN {1}sys.tables pk_tab WITH (NOLOCK) ON pk_tab.object_id = fk.referenced_object_id",
        "JOIN {1}sys.schemas fk_schema WITH (NOLOCK) ON fk_schema.[schema_id] = fk_tab.[schema_id]",
        "JOIN {1}sys.schemas pk_schema WITH (NOLOCK) ON pk_schema.[schema_id] = pk_tab.[schema_id]",
        "JOIN {1}sys.foreign_key_columns fk_cols WITH (NOLOCK) ON fk_cols.constraint_object_id = fk.object_id",
        "JOIN {1}sys.columns fk_col WITH (NOLOCK) ON fk_col.column_id = fk_cols.parent_column_id AND fk_col.object_id = fk_tab.object_id",
        "JOIN {1}sys.columns pk_col WITH (NOLOCK) ON pk_col.column_id = fk_cols.referenced_column_id AND pk_col.object_id = pk_tab.object_id",
        "{2}",
