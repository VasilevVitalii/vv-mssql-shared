...
function go (filter, allow_size) {
    ...
    let query_per_database = [
        ...
        "FROM {1}sys.views v WITH (NOLOCK)",
        "LEFT JOIN {1}sys.schemas s WITH (NOLOCK) ON v.schema_id = s.schema_id",
        allow_size === true ? "LEFT JOIN {1}sys.indexes i WITH (NOLOCK) ON v.object_id = i.object_id" : "",
