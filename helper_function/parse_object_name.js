    ...
    switch(part.length) {
        case 1:
            return {
                ...
                schema: 'dbo',
                object: vvs.isEmptyString(part[0]) ? '' : vvs.border_del(part[0].trim(), '[', ']')
            }
        case 2:
            return {
                base: vvs.toString(default_base, '').trim(),
                schema: schema(part[0]),
                object: vvs.isEmptyString(part[1]) ? '' : vvs.border_del(part[1].trim(), '[', ']')
            }
        case 3:
            ...
            return {
                ...
                object: vvs.isEmptyString(part[2]) ? '' : vvs.border_del(part[2].trim(), '[', ']')
            }
        default:
            return undefined
    }
